...
namespace EM.Plugins.TestPlugin.Handlers
{
    public class PlayerEventListener : IEventListener
    {
        ...
        public async ValueTask OnPlayerChat(IPlayerChatEvent e)
        {
            ...
            if (e.Message.StartsWith("/impostors") && e.ClientPlayer.IsHost)
            {
                string[] args = e.Message.Split(" ");
                e.Game.Options.NumImpostors = int.Parse(args[1]);
                await e.PlayerControl.SendChatToPlayerAsync("Impostors set.");
        ...
        {
            _logger.LogDebug("Murder event");
            e.Victim.NetworkTransform.SnapToAsync(new Vector2(0, 0));
        }
    }
}
